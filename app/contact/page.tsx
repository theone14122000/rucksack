...
export const metadata: Metadata = {
  ...
  description: "Reach out to Rucksack Adventures for travel enquiries, bookings, and custom itineraries. Located in Kasumpti, Shimla. Call, email, or WhatsApp us.",
};
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 mb-16">
          <div className="lg:col-span-2 space-y-6">
            {contactMethods.map((method) => (
              <div key={method.label} className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-card bg-brand-cream border border-brand-turquoise/12 flex items-center justify-center shrink-0">
                  <method.icon className="w-4 h-4 text-brand-turquoise" />
                </div>
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-brand-taupe mb-0.5">{method.label}</p>
                  {method.href ? (
                    <a href={method.href} target={method.href.startsWith("http") ? "_blank" : undefined} rel={method.href.startsWith("http") ? "noopener noreferrer" : undefined} className="text-sm text-brand-dark hover:text-brand-turquoise transition-colors">
                      {method.value}
                    </a>
                  ) : (
                    <p className="text-sm text-brand-dark">{method.value}</p>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="lg:col-span-3">
            <div className="bg-brand-cream/30 border border-brand-turquoise/12 p-6 sm:p-10 rounded-card-lg shadow-luxury">
              <div className="mb-6">
                <h2 className="font-editorial text-2xl font-bold text-brand-dark">Send Us an Enquiry</h2>
                <p className="text-xs text-brand-taupe mt-1">Fill in the details below. We typically respond within 2 hours.</p>
              </div>
