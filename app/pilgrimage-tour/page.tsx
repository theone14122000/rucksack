  ...
  return (
    <div className="pt-24 pb-20 bg-brand-cream">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Important Notice */}
        <div className="bg-brand-cream/60 border border-brand-turquoise/40 rounded-card p-6 mb-16 flex items-start gap-4">
          <AlertTriangle className="w-5 h-5 text-brand-turquoise shrink-0 mt-0.5" />
          <div>
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {circuits.map((circuit) => (
              <div
                key={circuit.name}
                className="bg-brand-cream/30 border border-brand-turquoise/30 rounded-card p-7 hover:border-brand-dark hover:shadow-luxury transition-all duration-300"
              >
                <h3 className="font-editorial text-xl font-bold text-brand-dark mb-3">
                  {circuit.name}
                </h3>
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-brand-taupe">Duration</p>
                    <p className="text-xs font-semibold text-brand-dark">{circuit.duration}</p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-brand-taupe">Difficulty</p>
                    <p className="text-xs font-semibold text-brand-dark">{circuit.difficulty}</p>
                  </div>
                </div>
                <p className="text-xs text-brand-dark/80 leading-relaxed">
                  {circuit.description}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20 py-12 border-t border-brand-turquoise/30">
          <div>
            <h2 className="font-editorial text-2xl font-bold text-brand-dark mb-6">
              What&apos;s Included
            </h2>
          ...
          <div>
            ...
            <div className="space-y-3">
              {requirements.map((item) => (
                <div key={item} className="flex items-start gap-3">
                  <AlertTriangle className="w-4 h-4 text-brand-turquoise shrink-0 mt-0.5" />
                  <span className="text-sm text-brand-dark">{item}</span>
                </div>
              ))}
            </div>
