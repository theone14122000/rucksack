...
export const TrustStrip: React.FC = () => {
  const stats = [
    {
      icon: <Award className="w-5 h-5 text-brand-brown" />,
      number: "8+ Years",
      label: "Of Mountain Heritage",
      caption: "Operating from Kasumpti, Shimla since 2018",
    },
    {
      icon: <Star className="w-5 h-5 text-brand-brown fill-brand-brown/20" />,
      number: "4.6 / 5.0",
    ...
    {
      icon: <Headset className="w-5 h-5 text-brand-brown" />,
      number: "24/7",
      label: "Service",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8 lg:gap-10 divide-y sm:divide-y-0 sm:divide-x divide-brand-brown/10">
          {stats.map((stat, idx) => (
            ...
            >
              <div className="mb-3 p-2 rounded-xs bg-brand-brown/5 border border-brand-brown/10">
                {stat.icon}
              </div>
              <span className="font-editorial text-3xl sm:text-4xl font-bold text-brand-black tracking-tight">
                {stat.number}
              </span>
